...
import { EFO_HEADERS, autoMapHeaders, normalizeEfoRow, toDbTransaction, type EfoHeader, type EfoRow } from "@/lib/efo-schema";
...
interface ParsedRow { row: number; data: EfoRow; errors: string[] }
...
function ImportPage() {
  ...
  function downloadTemplate() {
    const example1 = [
      "EXT-001","Empresa Alfa","12.345.678/0001-99","Receita","Serviços","CC-01","Consultoria mensal",
      "1500,00","0","0","0","simple","01/06/2026","10/06/2026","","","","Pix","Em aberto","Planilha","",
    ];
    const example2 = [
      "EXT-002","Cliente Beta","","Conta a Receber","Serviços","","Parcela 1/3",
      "500,00","500,00","0","2,5","compound","01/06/2026","12/jun/2026","","1","3","Boleto","Em aberto","Planilha","Contrato #123",
    ];
  ...
  async function ensureCompany(name: string, doc: string | null, ownerId: string, cache: Map<string, string>): Promise<string> {
    const key = name.toLowerCase().trim();
    if (cache.has(key)) return cache.get(key)!;
    const existing = companyByName.get(key);
    if (existing) { cache.set(key, existing.id); return existing.id; }
    const { data, error } = await supabase.from("companies").insert({ name, document: doc, owner_id: ownerId }).select("id").single();
    if (error) throw error;
  ...
  const importMut = useMutation({
    mutationFn: async () => {
      ...
      const batch = await supabase.from("import_batches").insert({
        owner_id: user.id, file_name: fileName, total_rows: parsed.length,
      ...
      if (invalid.length > 0) {
        await supabase.from("import_errors").insert(invalid.map((r) => ({
          ...
          error_message: r.errors.join("; "), raw_data: r.data as unknown as never,
        })) as never);
      }
      if (valid.length > 0) {
    ...
    onSuccess: () => {
      ...
      setParsed([]); setRawRows([]); setFileHeaders([]); setMapping({}); setFileName(""); setStep("upload");
  ...
  function downloadErrors() {
    const data = invalid.map((r) => ({ linha: r.row, erros: r.errors.join("; "), ...(r.data as unknown as Record<string, unknown>) }));
      ...
      {step === "mapping" && (
        <Card className="mb-4">
          <CardHeader>
            ...
            <p className="text-sm text-muted-foreground mt-1">Passo 2 de 3 — associe cada coluna do arquivo ao campo oficial do EFO. Campos não mapeados serão ignorados.</p>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-2"><Label>Ano padrão (para datas sem ano, ex: "12/jun")</Label>
                <Input type="number" value={defaultYear} onChange={(e) => setDefaultYear(Number(e.target.value))} />
              </div>
              <div className="space-y-2"><Label>Sistema origem (origem_sistema)</Label>
                <Input value={defaultSource} onChange={(e) => setDefaultSource(e.target.value)} placeholder="ex: planilha_antiga, erp_x" />
              </div>
            </div>
            <div className="overflow-x-auto">
              <Table>
                <TableHeader><TableRow><TableHead>Coluna do arquivo</TableHead><TableHead>Amostra</TableHead><TableHead>Campo EFO</TableHead></TableRow></TableHeader>
                <TableBody>
                  {fileHeaders.map((h) => (
                    <TableRow key={h}>
                      <TableCell className="font-medium">{h}</TableCell>
                      <TableCell className="text-xs text-muted-foreground max-w-xs truncate">{String(rawRows[0]?.[h] ?? "")}</TableCell>
                      <TableCell>
                        <Select value={mapping[h] || "__ignore__"} onValueChange={(v) => setMapping({ ...mapping, [h]: v === "__ignore__" ? "" : (v as EfoHeader) })}>
                          <SelectTrigger className="w-64"><SelectValue /></SelectTrigger>
                          <SelectContent>
                            <SelectItem value="__ignore__">— ignorar —</SelectItem>
                            {EFO_HEADERS.map((h2) => <SelectItem key={h2} value={h2}>{h2}</SelectItem>)}
                          </SelectContent>
                        </Select>
                      </TableCell>
                    </TableRow>
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => { setStep("upload"); setFileName(""); }}>Cancelar</Button>
            ...
            <div className="overflow-x-auto max-h-96">
              <Table>
                <TableHeader><TableRow>
                  <TableHead>Linha</TableHead><TableHead>Cliente</TableHead><TableHead>Tipo</TableHead><TableHead>Descrição</TableHead>
                  ...
                  {parsed.slice(0, 200).map((r) => {
                    ...
                    return (
                      <TableRow key={r.row} className={r.errors.length ? "bg-destructive/5" : ""}>
                        ...
                        <TableCell className="text-right">{d.valor_original.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</TableCell>
