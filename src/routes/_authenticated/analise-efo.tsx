...
function EFOPage() {
  ...
  const monthly = useMemo(() => {
    const rows = Array.from({ length: 12 }, (_, i) => ({
      mes: i + 1, label: new Date(year, i, 1).toLocaleDateString("pt-BR", { month: "short" }),
      receitas: 0, despesasFixas: 0, despesasVariaveis: 0, despesas: 0,
      resultado: 0, juros: 0, margem: 0,
    }));
  ...
  return (
    <>
      <PageHeader title="Análise Econômica, Financeira e Operacional" description="Comparativo mensal com margem e resultado do exercício." />

      <Card className="mb-4">
        <CardContent className="p-4 flex flex-wrap items-end gap-3">
          <div className="space-y-1"><label className="text-xs text-muted-foreground">Ano</label>
          ...
          <div className="space-y-1 min-w-52"><label className="text-xs text-muted-foreground">Cliente</label>
          ...
          <div className="ml-auto grid grid-cols-4 gap-4 text-right">
            ...
            <Kpi label="Resultado" value={formatBRL(totalResult)} color={totalResult >= 0 ? "text-success" : "text-destructive"} />
            <Kpi label="Margem" value={formatPercent(margemAvg)} color={margemAvg >= 0 ? "text-success" : "text-destructive"} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader><CardTitle>DRE mensal — {year}</CardTitle></CardHeader>
        <CardContent>
              ...
              <TableBody>
                {monthly.map((r) => (
                  <TableRow key={r.mes}>
                    ...
                    <TableCell className="text-right text-success">{formatBRL(r.receitas)}</TableCell>
                    ...
                    <TableCell className={`text-right font-medium ${r.resultado >= 0 ? "text-success" : "text-destructive"}`}>{formatBRL(r.resultado)}</TableCell>
                    <TableCell className={`text-right ${r.margem >= 0 ? "text-success" : "text-destructive"}`}>{formatPercent(r.margem)}</TableCell>
                  </TableRow>
                ))}
                <TableRow className="font-semibold bg-muted/40">
                  <TableCell>Total</TableCell>
                  <TableCell className="text-right text-success">{formatBRL(totalRec)}</TableCell>
                  <TableCell className="text-right">{formatBRL(monthly.reduce((s, r) => s + r.despesasFixas, 0))}</TableCell>
                  <TableCell className="text-right">{formatBRL(monthly.reduce((s, r) => s + r.despesasVariaveis, 0))}</TableCell>
                  <TableCell className="text-right text-warning">{formatBRL(totalJuros)}</TableCell>
                  <TableCell className={`text-right ${totalResult >= 0 ? "text-success" : "text-destructive"}`}>{formatBRL(totalResult)}</TableCell>
                  <TableCell className={`text-right ${margemAvg >= 0 ? "text-success" : "text-destructive"}`}>{formatPercent(margemAvg)}</TableCell>
                </TableRow>
              </TableBody>
            </Table>
          </div>
...
function Kpi({ label, value, color }: { label: string; value: string; color: string }) {
  return <div><div className="text-xs text-muted-foreground uppercase">{label}</div><div className={`text-lg font-semibold ${color}`}>{value}</div></div>;
}