...
function PlanoAcaoPage() {
  ...
  const { data: plans = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("action_plans_5w2h").select("*, companies(name)").order("when_date", { ascending: true, nullsFirst: false });
  ...
  const upsert = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["action-plans"] }); setOpen(false); setEditing(null); toast.success("Ação salva"); },
    onError: (e: Error) => toast.error(e.message),
  });

  const del = useMutation({
    mutationFn: async (id: string) => { const { error } = await supabase.from("action_plans_5w2h").delete().eq("id", id); if (error) throw error; },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["action-plans"] }); toast.success("Removido"); },
  });

  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const fd = new FormData(e.currentTarget);
    const g = (k: string) => { const v = fd.get(k); return v ? String(v) : null; };
    const payload: Partial<Plan> = {
      company_id: g("company_id"),
      what: g("what"), why: g("why"), who: g("who"),
      where_field: g("where_field"), how: g("how"),
  ...
  return (
    <>
      <PageHeader title="Plano de Ação 5W2H" description="What, Why, Who, Where, When, How, How much — organize e execute." actions={
        <Dialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) setEditing(null); }}>
          <DialogTrigger asChild><Button><Plus className="h-4 w-4 mr-2" />Nova ação</Button></DialogTrigger>
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle>{editing ? "Editar ação" : "Nova ação 5W2H"}</DialogTitle></DialogHeader>
            <form onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2 sm:col-span-2"><Label>O que fazer (What) *</Label><Textarea name="what" required defaultValue={editing?.what ?? ""} rows={2} /></div>
              <div className="space-y-2 sm:col-span-2"><Label>Por quê (Why)</Label><Textarea name="why" defaultValue={editing?.why ?? ""} rows={2} /></div>
              <div className="space-y-2"><Label>Quem (Who)</Label><Input name="who" defaultValue={editing?.who ?? ""} /></div>
              <div className="space-y-2"><Label>Onde (Where)</Label><Input name="where_field" defaultValue={editing?.where_field ?? ""} /></div>
              <div className="space-y-2 sm:col-span-2"><Label>Como (How)</Label><Textarea name="how" defaultValue={editing?.how ?? ""} rows={2} /></div>
              <div className="space-y-2"><Label>Quanto custa (How much)</Label><Input name="how_much" defaultValue={editing?.how_much ? String(editing.how_much) : ""} /></div>
              <div className="space-y-2"><Label>Quando (When)</Label><Input name="when_date" type="date" defaultValue={editing?.when_date ?? ""} /></div>
              <div className="space-y-2"><Label>Cliente</Label>
                <Select name="company_id" defaultValue={editing?.company_id ?? undefined}>
                  <SelectTrigger><SelectValue placeholder="Opcional" /></SelectTrigger>
                  <SelectContent>{(companies as { id: string; name: string }[]).map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              ...
              <DialogFooter className="sm:col-span-2"><Button type="submit" disabled={upsert.isPending}>Salvar</Button></DialogFooter>
            </form>
          </DialogContent>
        </Dialog>
      } />
                ...
                <TableRow key={p.id}>
                  ...
                  <TableCell>
                    <div className="flex gap-1">
                      {p.status !== "Concluído" && (
                        <Button size="icon" variant="ghost" title="Concluir" onClick={() => upsert.mutate({ id: p.id, status: "Concluído" })}>
                          <CheckCircle2 className="h-4 w-4 text-success" />
                        </Button>
                      )}
                      <Button size="icon" variant="ghost" title="Editar" onClick={() => { setEditing(p); setOpen(true); }}><Pencil className="h-4 w-4" /></Button>
                      <Button size="icon" variant="ghost" title="Excluir" onClick={() => { if (confirm("Remover?")) del.mutate(p.id); }}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                    </div>
                  </TableCell>
                </TableRow>
              ))}
...
function PriorityBadge({ p }: { p: string }) {
  const map: Record<string, string> = { "Urgente": "destructive", "Alta": "default", "Normal": "secondary", "Baixa": "outline" };
