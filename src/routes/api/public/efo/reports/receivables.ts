  ...
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        let q = admin.from("financial_transactions").select("*,companies(name)").eq("owner_id", auth.userId).in("movement_type", RECEIVABLE_TYPES as unknown as string[]);
        ...
        const rows = (data ?? []).map((t) => {
          ...
          return { id: t.id, cliente: (t as unknown as { companies?: { name: string } }).companies?.name, vencimento: t.due_date, valor_original: Number(t.original_value), juros: info.interest, valor_atualizado: info.updated, em_aberto: info.open, dias_atraso: info.days, status: t.status };
        });
